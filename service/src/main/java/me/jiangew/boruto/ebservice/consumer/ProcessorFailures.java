...
/**
 * Isolated failure management to support CodeTrans (generation of the example in the different
 * languages)
 *
  ...
  public static void handleFailure(Throwable t) {
    if (t instanceof ServiceException) {
      ...
      if (exc.failureCode() == ProcessorService.BAD_NAME_ERROR) {
        System.out.println(
            "Failed to process the document: The name in the document is bad. "
                + "The name provided is: "
                + exc.getDebugInfo().getString("name"));
