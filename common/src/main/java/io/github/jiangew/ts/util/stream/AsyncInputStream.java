...
package io.github.jiangew.ts.util.stream;
...
/**
 * Reads an input stream in an asynchronous and Vert.X compliant way. Instances
 * acts a finite state machine with 3 different states:
 * {@literal ACTIVE, PAUSED, 
 * CLOSED}. The transition between the states depends on the control flow (i.e.
 * the pump consuming the stream).
 */
public class AsyncInputStream implements ReadStream<Buffer> {
	...
	/**
	 * The failure handler called when an error is encountered while reading the
	 * stream.
	 */
	 ...
	 * This constructor uses the default chunk size.
	 * 
	 * @param vertx
	 *            the Vert.X instance
	 * @param executor
	 *            the executor used to read the chunk
	 * @param in
	 ...
	 * 
	 * @param endHandler
	 *            the handler called when the stream is read completely.
	...
	/**
	 * The method actually reading the stream. Except the first calls, this method
	 * is executed within an Akka thread.
	 */
	private void doRead() {
		if (context == null) {
	 ...
	 *         {@link org.wisdom.framework.vertx.AsyncInputStream} is in the
	 *         "CLOSED" state.
	 */
	 ...
	 * 
	 * @return {@code true} if we read the end of the stream, {@code false}
	 *         otherwise
	 * @throws Exception
	 ...
	 * Reads a chunk.
	 * 
	 * @return the read bytes, empty if we reached the end of the stream. The
	 *         returned array has exactly the sisize of the chunk.
	 * @throws Exception
	 *             if the stream cannot be read.
	 */
